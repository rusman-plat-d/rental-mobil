...
export class PenggunaTableDataSource extends DataSource<Pengguna> {
	 _filterChange = new BehaviorSubject('');
	get filter(): string { return this._filterChange.value; }
	set filter(filter: string) { this._filterChange.next(filter); }
	constructor(
		private _PenggunaDatabase: DatabaseService,
	...
	connect(): Observable<Pengguna[]> {
		const displayDataChanges = [
			this._paginator.page,
			this._sort.sortChange,
			this._PenggunaDatabase.dataChange,
			this._filterChange
		];
		return Observable.merge(...displayDataChanges).map(() => {
			...
			return this.getSortedData()
				...
				.filter((pengguna: Pengguna) => {
					let searchStr = (JSON.stringify(pengguna)).toLowerCase();
	...
	getSortedData(): Pengguna[] {
		const data = this._PenggunaDatabase.data.slice();
