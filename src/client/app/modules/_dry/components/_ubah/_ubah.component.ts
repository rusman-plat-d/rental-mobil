...
export class _UbahComponent implements AfterViewInit, OnInit {
	...
	constructor(
		...
		public $_pp2Database: DatabaseService<Mobil$Key | Pengguna$Key | Supir$Key>,
		public $_pp2: Pp2Service
	) {
		try{
			this.$key = this.$_ngActivatedRoute.params['value']['id'] || '';
		...
		if (this.jenis === 'mobil') {
			setTimeout(() => {
				...
				if ( this.$key != '' ) {
					...
					const mobil: Mobil$Key = this.$_pp2Database.data.filter(mobil => mobil.$key == this.$key)[0]
			...
			this.C_Mat_Select_Ubah_Mobil.valueChange.subscribe((mobil: string) => {
				this.C_Pp2_Dry_MobilForm.sembunyikan = false;
				const _mobil: Mobil$Key = JSON.parse(mobil);
				this.mobilSetValue(_mobil);
					this.$_ngRouter.navigate(['pengurus', 'mobil', 'ubah', _mobil.$key])
			})
			...
			setTimeout(() => {
				...
				if ( this.$key != '' ) {
					...
					const pengguna: Pengguna$Key = this.$_pp2Database.data.filter(pengguna => pengguna.$key == this.$key)[0]
			...
			this.C_Mat_Select_Ubah_Pengguna.valueChange.subscribe((pengguna: string) => {
				this.C_Pp2_Dry_PenggunaForm.sembunyikan = false;
				const _pengguna: Pengguna$Key = JSON.parse(pengguna);
				this.penggunaSetValue(_pengguna);
					this.$_ngRouter.navigate(['pengurus', 'pengguna', 'ubah', _pengguna.$key])
			})
		}else{
			setTimeout(() => {
				...
				if ( this.$key != '' ) {
					...
					const supir: Supir$Key = this.$_pp2Database.data.filter(supir => supir.$key == this.$key)[0]
			...
			this.C_Mat_Select_Ubah_Supir.valueChange.subscribe((supir: string) => {
				this.C_Pp2_Dry_SupirForm.sembunyikan = false;
				const _supir: Supir$Key = JSON.parse(supir);
				this.supirSetValue(_supir);
					this.$_ngRouter.navigate(['pengurus', 'supir', 'ubah', _supir.$key])
			})
		}
	}
	ngOnInit() {
	...
	penggunaSetValue(pengguna: Pengguna$Key){
		this.C_Pp2_Dry_PenggunaForm.sembunyikan = false;
