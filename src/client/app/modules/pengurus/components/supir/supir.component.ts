...
// alhamdulillahi afani mimaktalakabihi // menit 1:08
@Component({
	selector: 'pp2-pengurus-supir',
	styleUrls: [`supir.component.scss`],
	template: `
		<div class='pp2-pengurus-supir'>
			...
			<pp2-dry-container #C_Pp2_Dry_Container>
				<mat-tab-group #C_Mat_TabGroup backgroundColor="primary" color="accent" mat-stretch-tabs>
					<mat-tab (click)='$_ngRouter.navigate(["pengurus", "supir", "lihat"])'>
						<ng-template mat-tab-label>Lihat</ng-template>
					</mat-tab>
					<mat-tab (click)='$_ngRouter.navigate(["pengurus", "supir", "tambah"])'>
						<ng-template mat-tab-label>Tambah</ng-template>
					</mat-tab>
					<mat-tab (click)='$_ngRouter.navigate(["pengurus", "supir", "ubah"])'>
...
export class SupirComponent implements OnInit, AfterViewInit {
	...
	ngAfterViewInit(){
		...
		this.C_Mat_TabGroup.selectedIndexChange.subscribe((i)=>{
			this.$_ngRouter.navigate(['pengurus', 'supir', i == 0 ? 'lihat' : (i == 1 ? 'tambah' : 'ubah')])
		})
	}
	ngOnInit() {}
}
